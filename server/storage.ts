...
export interface IStorage {
  ...
  searchPhotos(terms: string[], startDate?: Date, endDate?: Date): Promise<Photo[]>;
  ...
  async searchPhotos(terms: string[], startDate?: Date, endDate?: Date): Promise<Photo[]> {
    return Array.from(this.photos.values()).filter(photo => {
      ...
      return terms.some(term => {
        ...
        if (photo.fileName.toLowerCase().includes(term.toLowerCase())) return true;
        
        // Check against location
        if (photo.location && photo.location.toLowerCase().includes(term.toLowerCase())) return true;
        
        // Check against content tags
        return photo.contentTags && photo.contentTags.some(tag => 
